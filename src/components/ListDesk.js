    ...
    <>
        ...
        {currentList.length > 0 && <div className="listDesk__currentContainer">
            ...
            <ul>
                {currentList?.map(({id, name, role, fromDate, toDate}) => {
                    return (
                        <li><ListItem idb={idb} desktop={true} listType="Current" name={name} role={role} key={id} id={id} fromDate={new Date(fromDate)} toDate={toDate && new Date(toDate)} cList={true}/></li>
                    )
                })}
            </ul>
        </div>}
        ...
        <h3>Previous Employees</h3>
            ...
            {previousList?.map(({id, name, role, fromDate, toDate}) => {
                    return (
                        <li><ListItem idb={idb} desktop={true} listType="Current" name={name} role={role} key={id} id={id} fromDate = {new Date(fromDate)} toDate = {new Date(toDate)} cList={false}/></li>
                    )
                })}
            </ul>
        </div>}
